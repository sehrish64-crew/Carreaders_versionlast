...
export function CountryProvider({ children }: { children: ReactNode }) {
  ...
  // Keep the site fixed to GBP only
  useEffect(() => {
    try {
  ...
  const setSelectedCountry = (_country: Country) => {
    // Currency is fixed to GBP, so ignore attempts to switch countries.
    if (process.env.NODE_ENV !== 'production') {
      console.log('[i18n] Currency fixed to GBP. Country switch ignored.')
    }
  }
