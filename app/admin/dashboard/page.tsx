...
export default function AdminDashboard() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-[#D0A0A0] text-[#1d4ed8] border border-[#D0A0A0]">Pending</span>
      case 'processing':
      ...
      case 'cancelled':
        return <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200">Cancelled</span>
      case 'refunded':
                      ...
                      <div className="flex gap-1 md:gap-2 flex-wrap">
                        {pendingTab ? (
                          <>
                            ...
                            <Button onClick={() => handleReject(review.id)} variant="outline" className="text-[#2563eb] border-[#D0A0A0] hover:bg-[#E8D0D0]" size="sm"><span className="hidden md:inline">Reject</span></Button>
                          </>
                        ) : (
                          <Button onClick={() => handleDelete(review.id)} variant="outline" className="text-blue-600 border-blue-200 hover:bg-blue-50" size="sm"><Trash2 className="w-3 md:w-4 h-3 md:h-4 mr-1" /><span className="hidden md:inline">Delete</span></Button>
                        )}
                      </div>
                    </td>
                  </tr>
