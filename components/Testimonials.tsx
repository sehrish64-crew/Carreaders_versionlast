...
export default function Testimonials() {
  ...
  const renderStars = (rating: number) => (
    <div className="flex gap-1 mb-4">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={`w-5 h-5 ${star <= rating ? 'fill-blue-400 text-blue-400' : 'text-gray-300'
            }`}
        />
      ))}
    </div>
  ...
  return (
    <section
      ref={sectionRef}
      className="relative py-20 md:py-28 bg-gradient-to-br from-blue-100 via-white to-sky-100 overflow-hidden"
    >
      <div className="absolute top-10 left-10 w-72 h-72 bg-[#2563eb] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
      <div className="absolute top-10 right-10 w-72 h-72 bg-[#2563eb] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000"></div>
      <div className="absolute bottom-10 left-1/2 w-72 h-72 bg-[#2563eb] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000"></div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`text-center mb-12 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          ...
          <Button 
            ...
            className="bg-blue-400 hover:bg-blue-500 text-slate-900 px-8 py-6 text-lg"
          >
        ...
        <div className={`relative h-80 rounded-3xl overflow-hidden mb-16 shadow-2xl transition-all duration-1000 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#2563eb]/60 via-[#2563eb]/30 to-transparent flex items-center">
            ...
            <div className="max-w-xl px-3">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex -space-x-3">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="w-12 h-12 rounded-full bg-[#2563eb] border-4 border-white flex items-center justify-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            {visibleReviews.map((review) => (
              <div key={review.id} className="bg-white p-8 rounded-2xl shadow-lg hover:-translate-y-2 transition">
                ...
                <Quote className="w-10 h-10 text-[#2563eb]" />

                {renderStars(review.rating)}

                <p className="text-gray-700 text-sm sm:text-base mb-6">{review.comment}</p>
